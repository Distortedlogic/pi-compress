 ...
 * manifest → src/index.ts. The other goldens load src/index.ts directly and
 * would not catch a broken root package entry point. Self-skips when pi is absent.
 */
...
const PI = piPath();
// EXTENSION_ENTRY = …/src/index.ts → up one directory to the repo root.
const REPO_ROOT = join(dirname(EXTENSION_ENTRY), "..");

describe.skipIf(!PI)("root pi-package entry loads in real pi", () => {
	it("pi -e <repo-root> resolves src/index.ts and registers the commands", { timeout: 120_000 }, async () => {
