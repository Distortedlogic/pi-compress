...
export async function undoHandler(pi: ExtensionAPI, ctx: ExtensionCommandContext): Promise<void> {
	...
	if (!step) {
		ctx.ui.notify("nothing to undo — no pi-compress mutation on the current branch", "info");
		return;
	}
...
export function registerBranch(pi: ExtensionAPI): void {
	...
	pi.registerCommand("branch", {
		description: "pi-compress: label this point and branch off (optionally onto a cheaper model)",
...
export function registerMerge(pi: ExtensionAPI, draft: DraftFn): void {
	pi.registerCommand("merge", {
		description: "pi-compress: close the open branch — squash (default) | --pick | --discard | --tournament",
...
export function registerUndo(pi: ExtensionAPI): void {
	pi.registerCommand("undo", {
		description: "pi-compress: revert the last mutation (re-open a branch / restore a crop) — append-only",
